...
<template>
  <section v-if="images && images.length > 0" class="space-y-4">
    ...
    <div
      ...
      <div class="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide aspect-video md:aspect-[21/9]">
        <div v-for="(img, idx) in images" :key="idx" class="snap-center shrink-0 w-full h-full relative cursor-zoom-in"
          @click="openLightbox(img)">
          <img :src="img" class="w-full h-full object-contain" loading="lazy" @error="handleImageError" />
    ...
    <Teleport to="body">
      <div v-if="fullscreenImage"
        ...
        @click="closeLightbox">
        <button class="absolute top-4 right-4 text-white hover:text-purple-400 transition" @click="closeLightbox">
          <Icon name="tabler:x" class="w-8 h-8" />
        </button>
        <img :src="fullscreenImage" class="max-w-full max-h-[90vh] rounded-xl shadow-2xl" @click.stop />
      </div>
    </Teleport>
  </section>
</template>

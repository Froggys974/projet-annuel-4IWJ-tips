  ...
  <div v-if="lat && lng">
    ...
    <div
      ...
      <button
        ...
        title="Agrandir la carte" @click="toggleMap">
        <Icon name="tabler:maximize" class="w-5 h-5" />
      </button>

      <div class="px-5 py-4 flex items-center justify-between">
        <span class="text-xs font-bold uppercase text-slate-500 dark:text-slate-400 flex items-center gap-2">
      ...
      <div class="h-48 rounded-3xl overflow-hidden relative z-0 opacity-90 hover:opacity-100 transition-opacity">
        <ClientOnly>
          <l-map ref="mapRef" :zoom="13" :center="[lat, lng]" :use-global-leaflet="false">
    ...
    <!-- Expanded Overlay -->
    <Teleport to="body">
      <div v-if="isMapExpanded" class="fixed inset-0 z-90 bg-white dark:bg-slate-900 flex flex-col animate-fade-in">
        <div
        ...
        <div class="flex-1 relative z-0">
          <ClientOnly>
            <l-map :zoom="15" :center="[lat, lng]" :use-global-leaflet="false">
              <l-tile-layer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" layer-type="base"
                name="OpenStreetMap" />
              <l-marker :lat-lng="[lat, lng]" />
            </l-map>

...
<template>
  <div class="flex flex-col gap-1.5">
    ...
    <div
      ...
      ]" @dragenter.prevent="dragActive = true" @dragleave.prevent="dragActive = false" @dragover.prevent
      @drop.prevent="onDrop" @click="triggerInput">
      <input
:id="id" ref="inputRef" type="file" :accept="accept" :multiple="multiple" class="hidden"
        @change="onChange">

      <div class="p-3 bg-purple-100 dark:bg-slate-800 rounded-full text-purple-600 dark:text-purple-400">
        <Icon name="tabler:cloud-upload" class="w-6 h-6" />
      </div>
      <div>
        <p class="font-bold text-slate-700 dark:text-slate-200">Cliquez ou déposez des fichiers ici</p>
    ...
    <div v-if="modelValue.length > 0" class="flex flex-col gap-2 mt-2">
      <div
v-for="(item, index) in previews" :key="index"
        ...
        <div class="w-10 h-10 rounded-lg overflow-hidden bg-slate-100 flex items-center justify-center shrink-0">
        ...
        <div class="flex-1 min-w-0 flex flex-col items-start truncate">
          <span class="text-sm font-medium text-slate-700 dark:text-slate-200 truncate w-full">{{ item.file.name
            }}</span>
          <span class="text-xs text-slate-400">{{ (item.file.size / 1024).toFixed(1) }} KB</span>
        </div>

        <button
type="button" class="p-1 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-lg transition-colors"

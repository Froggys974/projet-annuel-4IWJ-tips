...
<template>
  <div
    class="relative h-full w-full flex flex-col overflow-hidden rounded-3xl border border-purple-100/50 dark:border-slate-700/50 shadow-xl bg-white dark:bg-slate-900 transition-all duration-300 group">

    <!-- OVERLAY FILTRES (Glassmorphism Flottant) -->
    <div class="absolute top-4 left-4 right-4 z-[400] pointer-events-none flex justify-center md:justify-start">
      <div
...
icon-url="/pin.svg" :icon-size="[40, 40]" :icon-anchor="[20, 40]" :popup-anchor="[0, -40]"
            ...
            <LPopup :options="{ closeButton: false, offset: [0, -10] }">
              <div class="w-48 p-1 font-sans text-slate-800 dark:text-slate-900">
                <div class="text-[10px] font-bold text-purple-600 uppercase mb-1 tracking-wider">{{ tip.category }}
                </div>
